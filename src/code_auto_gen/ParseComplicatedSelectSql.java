...
public class ParseComplicatedSelectSql {
	...
	public static boolean parseSelect(String sql) throws Exception {
		...
		if (from_stmt.length() > 0) {
			...
			for (int i = 0; i < tablesAndAliass.length; i++) {
				...
				columns[i] = ColumnUtil.selectColumn(tables[i]);
			}
		}
	...
	public static String createSql(String sql) {
		try {
			...
			ParseComplicatedSelectSql.initVar();
			boolean flag = ParseComplicatedSelectSql.parseSelect(sql);
			System.out.println("declare_var2=" + ParseComplicatedSelectSql.declare_var);
			if (flag) {
				System.out.println("declare_var3=" + ParseComplicatedSelectSql.declare_var);
				return "";
