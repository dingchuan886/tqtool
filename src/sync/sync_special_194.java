...
public class sync_special_194 {

	private static final String tomcat = "tomcat1";

	private static final boolean ftl_boo = false;
	private static final boolean class_boo = false;
	private static final boolean html_boo = true;

	private static final String contain_ftl = "ftl|";// jar|xml|jsp|json|css|eot|svg|ttf|woff|jpg|png|js|
	private static final String contain_class = "class|xml|properties";
	private static final String contain_html = "|js|xml|jpg|png|gif";// |js|xml|jpg|png|gif";	//注意：html和js不能轻易更新
	

	// 设置主机ip，端口，用户名，密码
	private static Map<String, String> sftpDetails_48;
	private static Map<String, String> sftpDetails_194;
	private static Map<String, String> sftpDetails_149;
...
//		//
//		sftpDetails_149 = new HashMap<String, String>();
//		sftpDetails_149.put(SFTPConstants.SFTP_REQ_HOST, "182.254.131.149");
//		sftpDetails_149.put(SFTPConstants.SFTP_REQ_USERNAME, "root");
//		sftpDetails_149.put(SFTPConstants.SFTP_REQ_PASSWORD, "qiandao@315");
//		sftpDetails_149.put(SFTPConstants.SFTP_REQ_PORT, "22");
	}
	...
	public static void main(String[] args) throws Exception {
		...
		// html
		if (html_boo) {
			src = "D:\\Apache2.2\\chetuan_html\\special\\"; // 本地文件名
			dst = "/data/app/apache2/chetuan_html/special/"; // 目标文件名
		...
		// ftl
		if (ftl_boo) {
			src = "D:\\workspace_java\\pay\\WebContent\\"; // 本地文件名
			dst = "/data/app/" + tomcat + "/webapps/pay/"; // 目标文件名
			putDirectory(src, dst, chSftp, contain_ftl);
		}

		// class
		if (class_boo) {
			src = "D:\\workspace_java\\pay\\build\\classes\\"; // 本地文件名
			dst = "/data/app/" + tomcat + "/webapps/pay/WEB-INF/classes/"; // 目标文件名
		...
		**/
		...
		// ftl
		if (ftl_boo) {
			src = "D:\\workspace_java\\adminCT\\WebContent\\"; // 本地文件名
			dst = "/data/app/" + tomcat + "/webapps/adminCT/"; // 目标文件名
			putDirectory(src, dst, chSftp, contain_ftl);
		}

		// class
		if (class_boo) {
			src = "D:\\workspace_java\\adminCT\\build\\classes\\"; // 本地文件名
			dst = "/data/app/" + tomcat + "/webapps/adminCT/WEB-INF/classes/"; // 目标文件名
			putDirectory(src, dst, chSftp, contain_class);
			restart = true;
		}
		...
		// html
		if (html_boo) {
			src = "D:\\Apache2.2\\ctadmin_html\\"; // 本地文件名
			dst = "/data/app/apache2/ctadmin_html/"; // 目标文件名
			putDirectory(src, dst, chSftp, contain_html);
		}
		**/
		/************************** chetuan ************************
		// ftl
		if (ftl_boo) {
			src = "D:\\workspace_java\\chetuan\\WebContent\\"; // 本地文件名
			dst = "/data/app/" + tomcat + "/webapps/chetuan/"; // 目标文件名
			putDirectory(src, dst, chSftp, contain_ftl);
		}

		// class
		if (class_boo) {
			src = "D:\\workspace_java\\chetuan\\build\\classes\\";
			dst = "/data/app/" + tomcat + "/webapps/chetuan/WEB-INF/classes/";
			putDirectory(src, dst, chSftp, contain_class);
			restart = true;
		}
		...
		// html
		if (html_boo) {
			src = "D:\\Apache2.2\\chetuan_html\\";
			dst = "/data/app/apache2/chetuan_html/";
			putDirectory(src, dst, chSftp, contain_html);
		}
**/
		// 重启tomcat
